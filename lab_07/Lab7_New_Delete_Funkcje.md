...
Proszę napisać strukturę listy, przetrzymującą wartość typu `int` oraz `double`, w której każdy element posiada wskaźnik na poprzedni i ostatni element. Element pierwszy ma wskaźnik na poprzedni NULL. Element ostatni ma wskaźnik na ostatni NULL. Proszę zaimplementować dwie funkcje. Obie posiadają trzy argumenty: wskaźnik do struktury, `int`, `double`. Pierwsza dodaje element przed wskazanym przez wskaźnik, druga jako ostatni element struktury, bez względu na to na co wskazuje wskaźnik. Dodatkowo proszę zaimplementować funkcję wyświetlającą wszystkie elementy listy.
